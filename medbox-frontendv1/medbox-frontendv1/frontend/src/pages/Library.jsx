...
export default function Library() {
  ...
  const submit = async (e) => {
    ...
    if (editingId) {
      ...
      const advisory = await addMedicine({ ...form, instructions });
      if (advisory?.hasConflict) {
        pushToast(`${form.name} added. ⚠️ Drug advisory: ${advisory.advisoryMessage}`, "error");
      } else {
  ...
  const doDelete = async () => {
    ...
    await removeMedicine(confirmDelete.id);
